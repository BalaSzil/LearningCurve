...
class Guess:
    ...
    def check_if_previous_guess(self):
    ...
    def check_if_full_correct_guess(self):
    ...
    def check_if_guess_too_long(self):
        if self.check_if_full_correct_guess() is False and len(self.guess) != 1:
            self.tries -= 1
            print(f"Either guess the full word correctly, or take it one letter at a time, buddy. You have {self.tries} guesses remaining!\n")
            return True
    
    def check_if_guess_is_alphabetic(self):
    ...
    def validate(self, guess):
        ...
        if self.check_if_previous_guess():
            return False
        elif self.check_if_guess_too_long():
            return False
        elif self.check_if_guess_is_alphabetic():
            return False
        else:
            return True
    ...
    def run_game(self):
        ...
        while self.tries > 0:
            ...
            if self.validate(self.guess):               
                if self.check_if_full_correct_guess() is True:
